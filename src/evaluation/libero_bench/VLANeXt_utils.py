...
def get_vla(cfg):
    ...
    # Allow overriding attn impl from eval config (flash-attn may be unavailable).
    attn_implementation = "flash_attention_2"
    if hasattr(cfg, "model") and hasattr(cfg.model, "attn_implementation"):
    ...
    if model_type == 'rt2_baseline':
        ...
        model = VLANeXt(
            ...
            policy_mixer_type=model_config.get('policy_mixer_type', 'attention'),
            policy_mix_every_n=model_config.get('policy_mix_every_n', 4),
            policy_ttt_causal=model_config.get('policy_ttt_causal', False),
            policy_ttt_chunk_size=model_config.get('policy_ttt_chunk_size', 64),
            use_proprio_input_vlm=model_config.get('use_proprio_input_vlm', True),

...
def _mixer_at(layer_idx, depth, mixer_type, mix_every_n):
    """Return the mixer_type for layer `layer_idx` (0-based) under the [A,A,A,T]
    interleave: every `mix_every_n`-th layer (1-based) uses `mixer_type`, the
    rest use plain attention. With mixer_type='attention' this is all-attention
    (baseline, zero behavior change)."""
    if mixer_type == "attention" or mix_every_n is None or mix_every_n <= 0:
        return "attention"
    return mixer_type if ((layer_idx + 1) % mix_every_n == 0) else "attention"
...
class MoEBlock(nn.Module):
    def __init__(self, hidden_size, vlm_hidden_size, num_heads, mlp_ratio=4.0, gen_hidden_size=None,
                 mixer_type="attention", ttt_causal=False, ttt_chunk_size=64, layer_idx=0):
        super().__init__()
        ...
        if mixer_type == "attention":
            ...
            # Method-B TTT mixer replaces only the inner attention. It reads the
            # SAME context (vlm_proj/gen_proj outputs) the attention KV uses, so
            # the VLM-injection semantics are identical.
            self.attn = FastWeightGluMLPMultihead(
                dim=hidden_size, head_dim=hidden_size // num_heads,
                causal=ttt_causal, chunk_size=ttt_chunk_size,
                vlm_hidden_size=hidden_size,  # ctx already projected to hidden_size
            )
        elif mixer_type in ("gla", "gdn"):
            # Method-B linear-attention mixer (GatedLinearAttention / GatedDeltaNet):
            # ctx is prepended to the input sequence, the causal recurrence runs over
            # [ctx ; x], and only the x outputs are kept. Same VLM-injection semantics.
    ...
    def forward(self, x, c, vlm_feat, gen_feat=None):
        ...
        if gen_feat is not None and self.gen_proj is not None:
            ...
            ctx_list.append(g_feat)

        if self.mixer_type == "attention":
            kv = torch.cat([x_norm] + ctx_list, dim=1)
            attn_out, _ = self.attn(query=x_norm, key=kv, value=kv)
        else:  # ttt / gla / gdn: same context, fed as method-B injection
            ctx = torch.cat(ctx_list, dim=1)
            attn_out, _ = self.attn(x_norm, {}, ctx)
...
class ActionDiffusionTransformerMoE(nn.Module):
    def __init__(self, action_dim, vlm_hidden_size, hidden_size=384, depth=12, num_heads=6, mlp_ratio=4.0, gen_hidden_size=None,
                 mixer_type="attention", mix_every_n=4, ttt_causal=False, ttt_chunk_size=64):
        super().__init__()
        self.input_proj = nn.Linear(action_dim, hidden_size)
        self.t_embedder = TimestepEmbedder(hidden_size)
        ...
        self.blocks = nn.ModuleList([
            MoEBlock(hidden_size, vlm_hidden_size, num_heads, mlp_ratio=mlp_ratio, gen_hidden_size=gen_hidden_size,
                     mixer_type=_mixer_at(i, depth, mixer_type, mix_every_n),
                     ttt_causal=ttt_causal, ttt_chunk_size=ttt_chunk_size, layer_idx=i)

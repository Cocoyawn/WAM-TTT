...
try:
    # These tensor-parallel APIs are only available on torch >= 2.4 and are only
    # used by the (unused-in-VLANeXt) parallel ParallelStyle classes below.
    from torch.distributed import DeviceMesh
    from torch.distributed.tensor import Replicate, Shard, distribute_module
    from torch.distributed.tensor.parallel import ParallelStyle
except (ImportError, ModuleNotFoundError):
    DeviceMesh = None
    Replicate = Shard = distribute_module = None

    class ParallelStyle:
        """Stub: torch tensor-parallel API unavailable on this torch version."""
        pass

from src.models.fla.utils import autotune_cache_kwargs, get_multiprocessor_count, input_guard

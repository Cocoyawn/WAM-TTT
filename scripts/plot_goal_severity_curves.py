#!/usr/bin/env python
"""Robustness-vs-perturbation-strength curves for LIBERO-GOAL: TTT-256 vs Attention.
Adapted from scripts/plot_severity_curves.py (same Morandi palette / serif style / 5-subplot
layout), but: goal suite, only TTT-256 (warmup-aligned) + Attention, 2 lines.
Data: TTT-256 = dev cam/bg logs + /tmp/tttwu45_lnr_eval.json ; Attention = attncam20k + attnobj4d.
Saves docs/goal_severity_curves.png
"""
...
def cat_of(i): return CMAP.get(GB[i]['category']) if 0 <= i < len(GB) else None
def diff_of(i): return GB[i]['difficulty_level']
...
# TTT-256 (warmup-aligned) and Attention: goal 5-dim eval logs (8-way shards each)
ttt256 = collect('config/libero_plus_envgen_tttgoalwu_5dim_shard*.yaml')
att    = collect('config/libero_plus_envgen_attngoal_5dim_shard*.yaml')
...
plt.tight_layout()
out = "docs/goal_severity_curves.png"

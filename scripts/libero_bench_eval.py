...
def eval_libero(cfg) -> None:
    ...
    # Task sharding: if cfg.eval.task_ids is given, only run those task ids
    # (enables splitting the suite across multiple GPUs). Default = all tasks.
    ...
    print(f"Running task ids: {task_id_list}")
    ...
    for task_id in tqdm.tqdm(task_id_list):
        ...
        # Per-task result JSON (for merging sharded runs across GPUs).
        try:
            import json as _json
            with open(eval_dir / f"task_{task_id}_result.json", "w") as _jf:
                _json.dump({"task_id": int(task_id),
                            "successes": int(task_successes),
                            "episodes": int(task_episodes),
                            "success_rate": task_sr}, _jf)
        except Exception as _e:
            print(f"Failed to write per-task json: {_e}")

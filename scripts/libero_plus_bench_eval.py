...
from src.datasets.libero_act_old import (
...
def eval_libero(cfg) -> None:
    ...
    json_path = Path(benchmark.__file__).parent / "task_classification.json"
    ...
    # Optional task sharding: cfg.eval.task_ids restricts which task ids to run
    # (enables splitting the 1600+ environment-generalization tasks across GPUs).
    # Default = all tasks in the suite.
    task_ids_cfg = getattr(cfg.eval, "task_ids", None)
    if task_ids_cfg is not None and len(task_ids_cfg) > 0:
        task_id_list = [int(t) for t in task_ids_cfg]
    else:
        task_id_list = list(range(num_tasks_in_suite))
    print(f"Running {len(task_id_list)} task ids (first/last: "
          f"{task_id_list[0]}..{task_id_list[-1]})")
    ...
    # Per-category result JSON (for merging sharded runs across GPUs).
    try:
        with open(eval_dir / f"category_stats{('_' + shard_tag) if shard_tag else ''}.json", "w") as _jf:
            json.dump({
                "shard_tag": shard_tag,
        ...
        print(f"Failed to write category json: {_e}")
